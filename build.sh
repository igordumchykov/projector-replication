...
docker exec database_slave_1 sh -c "$start_slave_cmd"
docker exec database_slave_1 sh -c "mysql -u root -pS3cret -e 'SHOW SLAVE STATUS \G'"

docker exec database_slave_2 sh -c "$start_slave_cmd"
docker exec database_slave_2 sh -c "mysql -u root -pS3cret -e 'SHOW SLAVE STATUS \G'"